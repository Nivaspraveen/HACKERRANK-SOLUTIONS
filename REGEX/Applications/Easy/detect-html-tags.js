...
// In the above case, a is the tag name and href is an attribute having the value https://www.google.com.
...
function processData(input) {
    const lines = input.split('\n'),
        ...
        tagRegex = /<\s*\/?\s*([a-zA-Z0-9]+)(\s|>)/g,
        uniqueTags = new Set();
    for (let i = 1; i <= N; i++) {
        const line = lines[i];
        let match;
        while ((match = tagRegex.exec(line)) !== null) uniqueTags.add(match[1].trim());
    }
